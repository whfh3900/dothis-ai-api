...
class ParallelTimedRotatingFileHandler(logging.handlers.TimedRotatingFileHandler):
    def __init__(
        self, filename, when="h", interval=1, backupCount=0, encoding=None, delay=False, utc=False, postfix=".log"
    ):
        """
        Custom Log TimedRotatingFileHandler
        """
...
if BASE_DIR is None:
    ...
    custom_handler = ParallelTimedRotatingFileHandler(
        filename="log/dothis-fastapi-ai", when="midnight", interval=1, encoding="utf-8"
    )
    custom_handler.setFormatter(formatter)
    logger.addHandler(custom_handler)
else:
    os.environ["MODE"] = "lambda"


def api_logger(request, res_body, level=logging.INFO):
    logger.log(level=level, msg=request.url.path)
    if level == logging.DEBUG:
        logger.log(level=level, msg=res_body)