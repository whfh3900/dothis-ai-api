...
from dothis_nlp import PostProcessing, remove_special_characters
import math
...
class TfidfExtract(PostProcessing):
    ...
    def tfidf_extract(self, docs, threshold_1=0.8, threshold_2=3, ntop=30, keyword_max=None, use_upper=True):
        ...
        try:
            ...
            logger.error(f"tfidf_extract error: {e}")
            return list()
...
def bigkinds_related(keyword, time_date, topn=10):
    ...
    access_key = env.get('NEWS_API_ACCESSKEY')
    ...
    try:
        ...
        logger.error(f"bigkinds_related error: {e}")
        return list()
...
class GensimRelated(PostProcessing):
    ...
    def gensim_related(self, keyword, ntop=None, split_word_check=False):
        try:
            ...
            rel_words_list = self.post_processing([key[0].upper() for key in rel_words_tuple])
            ...
            logger.error(f"gensim_related error: {e}")
            return dict()
...
class WeeklyKeywords():
    def __init__(self, now_date=None, subscribers=1000000) -> None:
        if isinstance(now_date, str):
            self.now_date_str = now_date
            self.now_date = datetime.strptime(self.now_date_str, "%Y-%m-%d")
        elif isinstance(now_date, datetime):
            self.now_date = now_date
            self.now_date_str = datetime.strftime(self.now_date, "%Y-%m-%d")
        else:
        ...
        host = os.environ.get('MYSQL_HOST') # PROXMOX 엔드포인트 URL 또는 IP 주소
        port = int(os.environ.get('MYSQL_PORT')) # PROXMOX 데이터베이스 포트 (기본값: 3306)
        ...
        #### REDIS 연결 정보
        host = os.environ.get('REDIS_HOST') # RDS 엔드포인트 URL 또는 IP 주소
        port = int(os.environ.get('REDIS_PORT')) # RDS 데이터베이스 포트 (기본값: 3306)
        user = os.environ.get('REDIS_USER') # MySQL 계정 아이디
        password = os.environ.get('REDIS_PW') # MySQL 계정 비밀번호
        db = 0
    ...
    def calc_weekly_views(self, keyword, day_range=7, size=10000):
        ...
        if len(df) == 0:
            ...
            for date in self.get_week_dates(day_range=day_range):
                ...
                df['video_views'] = df.video_id.progress_apply(lambda x: self.video_view_count(x))
...
class Predicate():
    def __init__(self, stopwords_path="../usedata/stopwords_for_predicate.txt",
        ...
        if not match_poss:
            self.match_poss = [["XR", "XSA+EP"], ["VV+EP", "EF"], ["VV+EP", "ETM"],
                            ...
                            ["VV", "EP+EP"], ["NNG", "XSA+ETM"], ["VV+EC", "VX"], ["JKS", "VV+EC", "VX"], 
                            ["VV+EC", "VV+EC", "VX"], ["VV+EP", "EC"], ['NNG', 'XSV+EC'], ['NNG', 'NNG', 'XSA+ETM']]
    ...
    def get_data(self, keyword, related, days=31, size=20000):
...
if __name__ == "__main__":
    ...
    josa_path="usedata/kor_josa.txt"
    stopwords_path = "usedata/stopwords_for_related.txt"
    model="word2vec"
    # gr = GensimRelated(path=path, josa_path=josa_path, model=model)
    vb = VBR(josa_path=josa_path, stopwords_path=stopwords_path)
    
    text = "한문철"
    # result = gr.gensim_related(text, split_word_check=True)
    # print(result)

    # # print(list(result.keys()))
    # # print(list(result.values()))
    print(vb.related(text, size=10000))
