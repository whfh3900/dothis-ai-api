...
def calculate_korean_ratio(text, ratio=.0):
    ...
    pattern = r'(http|ftp|https)://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+' # URL제거
    text = re.sub(pattern=pattern, repl=' ', string=text)
...
def calculate_execution_time(func, *args, **kwargs):
    ...
    logger.info(f"Execution time: {int(hours)}시간 {int(minutes)}분 {seconds:.2f}초")
    return result
...
def decode_and_convert(s):
    ...
    if isinstance(s, str):
        ...
        try:
            ...
            logger.error(f"Error parsing {s}: {e}")
            return s
    ...
    def __init__(self, model="taeminlee/gliner_ko", 
        ...
        if use_cuda:
            if self.device == "cuda":
                ...
                logger.warning("cuda를 지원하지 않습니다.")
...
if __name__ == "__main__":
    stopwords_path = "../../data/stopwords/stopwords_for_redis.txt"
    use_cuda=True
    cache_dir = "../../models/huggingface"
    model_name = "/home/dothis/dothis-ai/code/preprocessing/GLiNER/examples/models"
    ...
    tta_labels = ["tokens", "artifacts", "person", "animal", "CIVILIZATION", "organization", \
            ...
            "landline phone number", "blood type", "cvv", "Food", "Brand", "resource", \
            "digital signature", "social media handle", "license plate number", "cnpj", "postal code", \
    ...
    print(posp.post_processing("옛사랑", use_stopword=True))
